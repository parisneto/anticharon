...
## Amendment (2026-09-17, pricing-engine-v2 / PE2-004, PE2-009)

This ADR remains **Accepted** for its core decision (adopting `effective-pricing` as the historical bootstrapper, and the internal-route resilience/degradation contract in Decision §4). Two sub-decisions above were superseded during the `pricing-engine-v2` initiative and did not ship as originally written here; this amendment reconciles the record rather than silently rewriting the sections above.

- **Decision §2 ("Dual-Tier Storage Architecture") is superseded.** `fullhistory.csv` — a per-provider, per-endpoint granular time-series carrying provider identity and token flow — was never implemented. The shipped design instead persists one collapsed cheapest-blended-price-per-day observation per model in `effective_prices.json` (no provider identity, no per-provider token flow); `history.csv` remains the compact sliding-window summary, unchanged in role. Full rationale, the approved reduction rule, and the deferred provider-granular alternative are documented in `docs/plans/pricing-engine-v2/PLAN.md`'s "Storage architecture" → "Scope correction" note and parked in `docs/plans/pricing-engine-v2/EXECUTION_CONTRACT.md`'s Deferred section (also tracked in `docs/BACKLOG.md`).
- **Decision §3 ("Smart Triggering Policy") is corrected to match the shipped trigger.** The originally decided triggers — a price-spike-driven resync, a 7-day staleness threshold, and an explicit `anticharon sync --full` command — were not implemented as described. The `model import-hermes` command's `sync` alias is a distinct, unrelated feature (one-way Hermes shortlist import) and does not perform a full price-history resync. What actually ships: `sync_effective_prices_for_model()` (`src/anticharon/tracker.py`) refreshes a model's granular history unconditionally on every `anticharon run`/`check`, but is a no-op unless that model's stored entry is older than `DEFAULT_EFFECTIVE_PRICES_STALE_HOURS` (24 hours, `src/anticharon/storage.py`) — there is no separate spike-triggered path and no user-facing `--full` flag. Routine fast checks and full-history refresh both go through the same internal per-endpoint stats route (§"Policy (ZDR) Pricing Data Source" in `docs/specs/spec_v1_anticharon.md` §3.2), not the public single-model REST endpoint originally described here.
- Decision §1 and §4 are unaffected and remain accurate as shipped.
