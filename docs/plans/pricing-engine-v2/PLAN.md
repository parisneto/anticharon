...
- Structure: `conftest.py`, `fixtures/` (sanitized real JSON payloads — the live-captured `/models/{slug}/endpoints` payload referenced in `ADR_CANDIDATE_TOKENS_CACHED.md`, a live-captured `/api/frontend/v1/stats/endpoint` payload, and (added 2026-09-17, PE2-006) a live-captured, trimmed-to-5-days `/api/frontend/v1/stats/effective-pricing` payload, all sanitized to remove any incidental private fields before committing, are strong starting fixtures), `test_log_parser.py`, `test_tracker.py`, `test_policy_pricing.py` (must include a fixture case where `dataPolicy.retainsPrompts` is `true` for every endpoint — the correct "fully unroutable under ZDR" behavior — since the live sample only showed a mix), `test_golden_pricing.py` (the 5 golden cases, verbatim), `test_analytics.py`, `test_effective_pricing_backfill.py` (includes the cross-validation canary, `@pytest.mark.live`, and the realistic-fixture parsing test), `test_network_failure_paths.py` (added 2026-09-17, PE2-003/PE2-006: deterministic timeout/HTTP/malformed-JSON/wrong-shape coverage for every network-facing fetch function), `test_mcp.py` (deterministic mocked coverage of `check_prices(zdr_only=True)`'s three-price payload, added 2026-09-17, PE2-006 — the original suite was live-only).
...
- **Negative-token-count validation hardening (PE2-006, deferred 2026-09-18):** `log_parser.py`'s per-row clamp (cached-exceeds-prompt, commit `d6dcc7e`) doesn't floor a raw negative `tokens_prompt`/`tokens_completion`/`tokens_cached` value at zero. Non-release-blocking deferral — synthetic edge case, not yet observed in a real export. See `docs/plans/pricing-engine-v2/RELEASE_VALIDATION.md#pe2-006` and `docs/BACKLOG.md`.
