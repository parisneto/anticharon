...
- **Hermes YAML fallback list dropped, destroying the synced shortlist (#4) (`src/anticharon/hermes.py`, `tracker.py`, `cli.py`, `mcp.py`, `tester.py`):**
  ...
  - `sync_hermes_to_config()` never overwrites an existing non-empty shortlist from an `incomplete` detection; a `complete` result remains authoritative and may legitimately shrink the shortlist.
  ...
  - **Independent-review corrections (GH-4):** `fetch_models_from_cli()`'s `fallback_providers` sub-query only checked `returncode == 0` before parsing; a non-zero exit fell through with `detection` still at its default `complete`, so a failed fallback sub-query after a successful default-model query produced the exact same silently-destructive default-only "complete" result the original fix was meant to eliminate. A non-zero fallback-sub-query exit is now `incomplete`, allowing the file tier to run, same as unparseable-but-non-empty output. Separately, `sync_hermes_to_config()`'s overwrite guard only compared list *lengths* (`len(new) < len(current)`), so an `incomplete` result with the same length as the existing shortlist (but different content) still overwrote it. The guard now rejects any `incomplete` result against an existing non-empty shortlist regardless of length — shorter, equal-length-different, and longer are all preserved the same way; only a `complete` result remains authoritative.
...
- **`anticharon check --zdr` broke the ASCII price spectrum chart (#3) (`src/anticharon/chart.py`):**
  ...
  - **Independent-review correction (GH-3):** a positive-infinity effective price (e.g. from a malformed upstream `"Infinity"` pricing string) crashed the chart entirely — `max(p.price_1m for p in ordered)` picked up the `inf`, that same entry's own ratio became `inf / inf` (`NaN`), and `int(round(nan))` raised `ValueError`. `anticharon.pricing.is_valid_listed_price()` now rejects `Infinity`/`-Infinity`/`NaN` at the shared pricing-validation boundary every tracker/discovery call site already gates on, so such prices can no longer reach normal tracker/chart output. `render_ascii_price_bar()` is also defensively hardened on its own: scale and the 🏆 [BEST] badge are now derived from the *finite* subset of prices only, and any non-finite entry that still reaches it renders a capped, non-crashing row instead.
