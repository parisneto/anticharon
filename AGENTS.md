...
- **Protocol baseline (planning stage):** Every sprint ledger (`docs/plans/<initiative>/EXECUTION_CONTRACT.md`) pins the MCP specification revision it builds against (version and URL; current baseline: MCP specification `2026-07-28`, https://modelcontextprotocol.io/specification/2026-07-28, also recorded in spec §10.1). Sprint planning starts with a protocol-update check for new MCP specification revisions (and relevant agent-to-agent protocol changes) since the pinned baseline; the check's result is recorded as a finding in that sprint's ledger before scope is closed. Pull-request descriptions cite the baseline.
